...
class Epsilon_decreasing_competitor(Competitor):	



	#initialization
	epsilon0=0.2 #probability that a random price is chosen
	...
	def __init__(self, competitor_number, epsilon0):
	...
	def p(self, prices_historical, demand_historical, t):#, parameterdump
		...
		if t==0: #if it is the first day
			...
			index=self.select_arm(self.epsilon0,self.values,t) #index is the price chosen at t
	...
	def select_arm(self, epsilon0,values,t): #it returns which arm to play
		epsilon=min(1,epsilon0/t)
